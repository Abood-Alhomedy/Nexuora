...
class ComponentController extends Controller
{
    ...
    ///
        // ① المستخدم يحفظ Widget خاص به للمجتمع
    public function saveUserWidget(Request $request)
    {
        ...
        $data = [
            'user_id'      => $user->id,
            'name'         => $request->name,
            'screen_data'  => $request->screen_data, // JSON الشاشة
            'is_community' => $request->is_community ?? 0,
            'status'       => 1,
        ];

        // حفظ صورة المعاينة إذا أرسلها المستخدم
        if ($request->hasFile('preview_image')) {
            $file = $request->file('preview_image');
            $path = $file->store('community_widgets', 'public');
            $data['preview_image'] = asset('storage/' . $path);
        }
    ...
    // ② قائمة الـ Widgets المجتمعية لجميع المستخدمين
    public function communityWidgetList(Request $request)
    {
        $list = Component::with('category')
        ...
        $list  = $list->orderBy('import_count', 'desc')->paginate(20);
        $items = ComponentResource::collection($list);
    ...
    // ③ استيراد Widget (يزيد عداد import_count)
    public function importWidget(Request $request)
    {
        $component = Component::find($request->id);
        if (!$component) {
            return response()->json(['status' => false, 'message' => 'Widget not found']);
        }

        $component->increment('import_count');

        return response()->json([
            'status'      => true,
            'screen_data' => $component->screen_data, // نرجع الـ JSON للفرونت
        ]);
    }
