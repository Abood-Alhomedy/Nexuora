 ...
 */
    ...
    public function downloadUserProject(Request $request)
    {
        ...
        foreach ($request->contents as $value) {
            $sanitizedContent = preg_replace_callback(
                '/Color\(alpha:\s*([\d.]+),\s*red:\s*([\d.]+),\s*green:\s*([\d.]+),\s*blue:\s*([\d.]+)(?:,\s*colorSpace:[^)]+)?\)/',
                function ($matches) {
                    $a = floatval($matches[1]);
                    $r = intval(round(floatval($matches[2]) * 255));
                    $g = intval(round(floatval($matches[3]) * 255));
                    $b = intval(round(floatval($matches[4]) * 255));
                    return "Color.fromRGBO($r, $g, $b, $a)";
                },
                $value['file_content']
            );
            Storage::disk('public')->put($user_folder_naming.'/lib/'.$value['file_name'], $sanitizedContent);
        }
    ...
    public function buildUserApk(Request $request)
    {
        ...
        $user_folder_naming = $user_id.'/'.$project_name;
 
        Storage::disk('public')->makeDirectory($user_folder_naming);

        $default_project_structure = storage_path('app/public/flutterviz');
        $user_project_folder = storage_path('app/public/'.$user_folder_naming);

        // Delete previous temp projects to save space
        if(is_dir(storage_path('app/public/'.$user_id))){
            try {
                deleteDirectory(storage_path('app/public/'.$user_id));
            } catch (\Exception $e) {}
        ...
        \File::copyDirectory($default_project_structure, $user_project_folder); 
        ...
        $project_path = $user_project_folder;
        $command = "cd \"$project_path\" && flutter build apk --release";
        ...
        // Copy generated APK
        $apk_source = $project_path . DIRECTORY_SEPARATOR . 'build' . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'outputs' . DIRECTORY_SEPARATOR . 'flutter-apk' . DIRECTORY_SEPARATOR . 'app-release.apk';
        
        $apk_dest_name = $user_id . '_' . $request->project_name . '_app.apk';
        $apk_dest = storage_path('app/public/' . $apk_dest_name);

        if (file_exists($apk_source)) {
            copy($apk_source, $apk_dest);
            $url = asset('storage/' . $apk_dest_name);
            
            // Cleanup temp project
            try {
                deleteDirectory($user_project_folder);
            } catch (\Exception $e) {}
        ...
        return response()->json([
            'status' => false,
            'message' => 'APK file not found after build'
        ]);
    }
