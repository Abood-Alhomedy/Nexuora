...
import 'package:nexuora/adminDashboard/model/add_category_response.dart';
import 'package:nexuora/adminDashboard/model/categoryList_model.dart';
import 'package:nexuora/adminDashboard/model/dashBoard_model.dart';
import 'package:nexuora/adminDashboard/model/delete_category_response.dart';
import 'package:nexuora/adminDashboard/model/feed_back_model.dart';
import 'package:nexuora/adminDashboard/model/project_list_model.dart';
import 'package:nexuora/adminDashboard/model/project_template_list_model.dart';
import 'package:nexuora/adminDashboard/model/template_list_model.dart';
import 'package:nexuora/adminDashboard/model/tutorials_model.dart';
import 'package:nexuora/adminDashboard/model/user_list_model.dart';
import 'package:nexuora/model/add_screen_model.dart';
import 'package:nexuora/model/base_response.dart' as FB;
import 'package:nexuora/model/category_component_list_model.dart';
import 'package:nexuora/model/category_template_list_model.dart';
import 'package:nexuora/model/city_model.dart';
import 'package:nexuora/model/country_model.dart';
import 'package:nexuora/model/login_response.dart';
import 'package:nexuora/model/media_list_model.dart';
import 'package:nexuora/model/screen_list_response.dart';
import 'package:nexuora/model/state_model.dart';
import 'package:nexuora/model/user_project_list_model.dart';
...
/// تشغيل المشروع على جهاز مباشر
Future<FB.BaseResponse> runProjectApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await (handleResponse(
      await buildHttpResponse('run-project', request: request, method: HttpMethod.POST)
    ))
  );
}

/// الحصول على قائمة الأجهزة المتاحة
Future<FB.BaseResponse> getDevicesApi() async {
  return FB.BaseResponse.fromJson(
    await (handleResponse(
      await buildHttpResponse('get-devices', method: HttpMethod.GET)
    ))
  );
}

/// اختيار جهاز معين للتشغيل
Future<FB.BaseResponse> selectDeviceApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await (handleResponse(
      await buildHttpResponse('select-device', request: request, method: HttpMethod.POST)
    ))
  );
}

/// إرسال تحديث مباشر إلى الجهاز المتصل
Future<FB.BaseResponse> liveUpdateApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await (handleResponse(
      await buildHttpResponse('live-update', request: request, method: HttpMethod.POST)
    ))
  );
}

/// إيقاف تشغيل المشروع على الجهاز
Future<FB.BaseResponse> stopProjectApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await (handleResponse(
      await buildHttpResponse('stop-project', request: request, method: HttpMethod.POST)
    ))
  );
}
...
/// اتصال ADB عبر Wi-Fi (IP:Port)
Future<FB.BaseResponse> wifiDeviceConnectApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('wifi-device/connect', request: request, method: HttpMethod.POST),
    ),
  );
}

/// قطع اتصال ADB
Future<FB.BaseResponse> wifiDeviceDisconnectApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('wifi-device/disconnect', request: request, method: HttpMethod.POST),
    ),
  );
}

/// قائمة الأجهزة المتصلة عبر ADB
Future<FB.BaseResponse> wifiDeviceListApi() async {
  return FB.BaseResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('wifi-device/devices', method: HttpMethod.GET),
    ),
  );
}

/// تشغيل المشروع على جهاز Wi-Fi
Future<FB.BaseResponse> wifiDeviceRunApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('wifi-device/run', request: request, method: HttpMethod.POST),
    ),
  );
}

/// Hot Reload
Future<FB.BaseResponse> wifiDeviceHotReloadApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('wifi-device/hot-reload', request: request, method: HttpMethod.POST),
    ),
  );
}

/// إيقاف تشغيل المشروع
Future<FB.BaseResponse> wifiDeviceStopApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('wifi-device/stop', request: request, method: HttpMethod.POST),
    ),
  );
}

/// بناء التطبيق كملف APK
Future<FB.BaseResponse> buildProjectApkApi(Map request) async {
  return FB.BaseResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('build-user-apk', request: request, method: HttpMethod.POST),
    ),
  );
}