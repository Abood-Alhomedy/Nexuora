          ...
          <Widget>[
            paddingView(
              ...
              padding: textButtonClass.padding,
                ...
                // أضف قسم (Action / Navigation) هذا:
        ExpansionTileView(
          "Navigation (أحداث الانتقال)",
          context,
          <Widget>[
            Container(
              ...
              child: DropdownButtonFormField<String>(
                decoration: InputDecoration(
                  labelText: 'اختر الصفحة المراد الانتقال اليها',
                  border: OutlineInputBorder(),
                  contentPadding: EdgeInsets.symmetric(horizontal: 10),
                ),
                value: textButtonClass.targetScreenName,
                // نقوم بجلب قائمة الشاشات التي أنشأها المستخدم المتواجدة في appStore
                items: appStore.screenList.map((screen) {
                  return DropdownMenuItem(
                    value: screen.name, // الخاصية تعتمد على اسم الشاشة
                    child: Text(screen.name ?? ''),
                  );
                }).toList(),
                onChanged: (value) {
                  // تحديث خصائص الزر وحفظها في الـ Store
                  textButtonClass.actionType = 'Navigate';
                  textButtonClass.targetScreenName = value;
                  setState(() {});
                  appStore.updateData(textButtonClass);
                },
              ),
