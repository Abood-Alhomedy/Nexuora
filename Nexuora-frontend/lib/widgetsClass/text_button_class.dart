...
class TextButtonClass {
  ...
  TextButtonClass.fromJson(Map<String, dynamic> json) {
    ...
    textColor = json['textColor'] != null
        ? fromJsonColor(json['textColor'])
        : DEFAULT_TEXT_COLOR;
    tFontSize = json['fontSize'] != null ? json['fontSize'] : DEFAULT_FONT_SIZE;
    backgroundColor = json['backgroundColor'] != null
        ? fromJsonColor(json['backgroundColor'])
        : DEFAULT_TEXT_BUTTON_BG_COLOR;
    elevation = json['elevation'] != null
        ? json['elevation']
        : DEFAULT_TEXT_BUTTON_ELEVATION;
    borderColor = json['borderColor'] != null
        ? fromJsonColor(json['borderColor'])
        : DEFAULT_TEXT_BUTTON_BORDER_COLOR;
    tFontWeight = json['tFontWeight'] != null
        ? json['tFontWeight']
        : FontWeightTypeNormal;
    contentPadding = json['contentPadding'] != null
        ? fromJsonPadding(json['contentPadding'])
        : EdgeInsets.symmetric(horizontal: 16, vertical: 16);
    tFontStyle = json['tFontStyle'] != null
        ? fromJsonFontStyle(json['tFontStyle'])
        : FontStyle.normal;
    borderRadius = json['borderRadius'] != null
        ? fromJsonBorderRadius(json['borderRadius'])
        : BorderRadius.zero;
    borderWidth = json['borderWidth'] != null
        ? json['borderWidth']
        : DEFAULT_BORDER_WIDTH;
    padding = json['padding'] != null
        ? fromJsonPadding(json['padding'])
        : EdgeInsets.zero;
    height = json['height'] != null
        ? fromJsonHeight(json['height'], heightType ?? TypePX)
        : DEFAULT_TEXT_BUTTON_HEIGHT;
    minWidth = json['minWidth'] != null
        ? fromJsonWidth(json['minWidth'], widthType ?? TypePX)
        : DEFAULT_TEXT_BUTTON_WIDTH;
    ...
    isHeightClear =
        json['isHeightClear'] != null ? json['isHeightClear'] : false;
    ...
    targetScreenName = json['targetScreenName'];
  }
  ...
  Map<String, dynamic> toJson() {
    ...
    if (this.targetScreenName != null) {
      data['targetScreenName'] = this.targetScreenName;
    }
    return data;
  }
  ...
  Widget getDefaultTextButtonWidget(WidgetModel widgetModel) {
    Widget childData = AbsorbPointer(
      absorbing:
          absorbPointer(), // هذا يمنع الضغط في وضع التصميم ويسمح به في المعاينة
      child: MaterialButton(
        onPressed: () {
          // التحقق مما إذا كانت هناك شاشة مستهدفة محددة
          if (targetScreenName != null && targetScreenName!.isNotEmpty) {
            // البحث عن بيانات الشاشة في قائمة الشاشات الخاصة بالمشروع
            var screens = appStore.screenList
                .where((element) => element.name == targetScreenName)
                .toList();

            if (screens.isNotEmpty) {
              // إذا تم إيجاد الشاشة المستهدفة، نقوم بحفظ التعديلات الحالية والانتقال الفوري لها
              savePreviousChanges(screens.first);
            } else {
              getToast("الشاشة المطلوبة غير موجودة");
            }
          } else {
            getToast("Button Press"); // الحدث الافتراضي في حال عدم تحديد شاشة
          }
        },
        color: backgroundColor ?? DEFAULT_TEXT_BUTTON_BG_COLOR,
        elevation: elevation ?? DEFAULT_TEXT_BUTTON_ELEVATION,
        shape: RoundedRectangleBorder(
          borderRadius: borderRadius ?? BorderRadius.zero,
          side: BorderSide(
            color: borderColor ?? DEFAULT_TEXT_BUTTON_BORDER_COLOR,
        ...
        child: Text(
          ...
          style: TextStyle(
            ...
            fontWeight: tFontWeight != null
                ? getFontWeight(tFontWeight)
                : FontWeight.normal,
            fontStyle: tFontStyle ?? FontStyle.normal,
          ),
        ),
        textColor: textColor ?? DEFAULT_TEXT_COLOR,
        height: isHeightClear!
            ? null
            : fromJsonHeight(height ?? DEFAULT_TEXT_BUTTON_HEIGHT, heightType),
        minWidth: isWidthClear!
            ? null
            : fromJsonWidth(minWidth ?? DEFAULT_TEXT_BUTTON_WIDTH, widthType),
      ),
    );
    return getGestureDetector(widgetModel, childData);
  }
  ...
  Widget getTextButtonWidget(WidgetModel widgetModel) {
    if (getExpanded(widgetModel, isExpanded) &&
        getPadding(padding) &&
        getHorizontalOrVerticalAlignment(
          horizontalAlignment,
          verticalAlignment,
          isAlignX,
          isAlignY,
        )) {
      return _getTextButtonExpanded(
        _getTextButtonPadding(
          _getTextButtonAlign(getDefaultTextButtonWidget(widgetModel)),
        ),
      );
    } else if (getExpanded(widgetModel, isExpanded) && getPadding(padding)) {
      return _getTextButtonExpanded(
        _getTextButtonPadding(getDefaultTextButtonWidget(widgetModel)),
      );
      ...
      return _getTextButtonPadding(
        _getTextButtonAlign((getDefaultTextButtonWidget(widgetModel))),
      );
    } else if (getPadding(padding)) {
      return _getTextButtonPadding(getDefaultTextButtonWidget(widgetModel));
    } else if (getHorizontalOrVerticalAlignment(
      horizontalAlignment,
      verticalAlignment,
      isAlignX,
      isAlignY,
  ...
  getMaterialButtonString() {
    ...
    if (targetScreenName != null && targetScreenName!.isNotEmpty) {
      // الكود الذي سيتم توليده للزر عند تصدير التطبيق
      onPressedCode =
          "() {\n  Navigator.push(context, MaterialPageRoute(builder: (context) => $targetScreenName()));\n}";
    }
