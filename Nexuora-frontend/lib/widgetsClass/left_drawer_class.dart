import 'package:nexuora/externalClasses/nexuora_drawer.dart';
...
class LeftDrawerClass {
  ...
  /// Drawer Items
  List<NexuoraDrawerItemModel> getNexuoraDrawerItems() {
    List<NexuoraDrawerItemModel> NexuoraDrawerItems = [];
    drawerItems.forEach((element) {
      NexuoraDrawerItems.add(NexuoraDrawerItemModel(icon: IconData(element!.icon!.codePoint!, fontFamily: "MaterialIcons"), label: element.label));
    });
    return NexuoraDrawerItems;
  }
  ...
  Widget getLeftDrawerWidget({BuildContext? context}) {
    return GestureDetector(
      child: NexuoraDrawer(
        ...
        drawerItems: getNexuoraDrawerItems(),
  ...
  getCodeAsString() {
    return "NexuoraDrawer(\n"
        ...
        "${email != "" ? "email:\"${email ?? DUMMY_USER_EMAIL}\",\n"
            ...
            "color:${emailFontColor ?? DEFAULT_DRAWER_FONT_COLOR},\n"
            "),\n" : ""}"
        "${getNexuoraDrawerItems().length != 0 ? "drawerItems: NexuoraDrawerItems,\n"
            "iconColor: ${iconColor ?? DEFAULT_ICON_COLOR},\n"
            "iconSize: ${iconSize ?? DEFAULT_ICON_SIZE},\n"
            "labelStyle: TextStyle(\n"
            "fontWeight: ${getFontWeight(labelFontWeight) ?? FontWeight.normal},\n"
  ...
  getHeaderClassFiles() {
    return ["import 'nexuora_drawerItem_model.dart';", "import 'nexuora_drawer.dart';"];
  }
}
